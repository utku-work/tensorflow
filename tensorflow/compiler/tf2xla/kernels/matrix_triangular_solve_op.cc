...
MatrixTriangularSolveOp::Broadcast(xla::XlaOp lhs, const TensorShape& lhs_shape,
  ...
  MaybeAddExpression(&lhs_broadcast_shape, xla::DynExpr::_(m));
  lhs_broadcast_shape.AddDim(m);
  MaybeAddExpression(&lhs_broadcast_shape, xla::DynExpr::_(m));
  auto lhs_output = BroadcastTo(lhs, lhs_broadcast_shape.dim_sizes(),
                                lhs_broadcast_shape.get_expressions_or_constants());
  if (!lhs_output.ok()) {
  ...
  MaybeAddExpression(&rhs_broadcast_shape, xla::DynExpr::_(m));
  rhs_broadcast_shape.AddDim(n);
  MaybeAddExpression(&rhs_broadcast_shape, xla::DynExpr::_(n));
  auto rhs_output = BroadcastTo(rhs, rhs_broadcast_shape.dim_sizes(),
                                rhs_broadcast_shape.get_expressions_or_constants());
  if (!rhs_output.ok()) {
